...
use rustls_mbedcrypto_provider::cipher_suite;
use rustls_mbedcrypto_provider::mbedtls_crypto_provider as default_provider;
...
fn make_client_config(params: &BenchmarkParam, clientauth: ClientAuth, resume: ResumptionParam) -> ClientConfig {
    ...
    let cfg = ClientConfig::builder_with_provider(
        CryptoProvider { cipher_suites: vec![params.ciphersuite], ..default_provider() }.into(),
    )
    .with_protocol_versions(&[params.version])
    .unwrap()
