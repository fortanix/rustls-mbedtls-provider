...
use super::aead::{
    self, Algorithm, AES128_GCM, AES256_GCM, GCM_EXPLICIT_NONCE_LEN, GCM_FIXED_IV_LEN,
    GCM_OVERHEAD,
};
    ...
    fn extract_keys(
        ...
        match self.cipher_type {
            CipherType::Aes128Gcm => Ok(ConnectionTrafficSecrets::Aes128Gcm {
                key,
                iv: gcm_iv(iv, explicit),
            }),
            CipherType::Aes256Gcm => Ok(ConnectionTrafficSecrets::Aes256Gcm {
                key,
                iv: gcm_iv(iv, explicit),
            }),
